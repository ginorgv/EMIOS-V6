//
// Funciones de informes fichero de consumos y costes (de SmartMeter) (electricidad)
//


// Genera el informe fichero de consumos y costes por tramo
function smartmeter_consumos_costes_tramos_ver_informe_fichero_electricidad(parametros_informe) {
    ...
    if (error_parametros == true) {
        $('#mensaje-aviso-informe-fichero-consumos-costes-tramos').html(
        ...
        elimina_elementos([
            "pagina-informe-fichero-consumos-costes-tramos-consumos",
            "pagina-informe-fichero-consumos-costes-tramos-costes"]);
        return;
    }
    ...
    function (data, status) {
        ...
        $('#titulo-informe-fichero-consumos-costes-tramos-consumos').html(TLNT.Idiomas._("Consumos por tramo"));
        $('#titulo-informe-fichero-consumos-costes-tramos-costes').html(TLNT.Idiomas._("Costes por tramo"));
...
function smartmeter_cortes_tension_ver_informe_fichero_electricidad(parametros_informe) {
    // Se recuperan los parámetros del informe
    var error_parametros = parametros_informe["error_parametros"];
    var descripcion_error_parametros = parametros_informe["descripcion_error_parametros"];
    ...
    // Si hay error en el informe se muestra un mensaje de aviso y se eliminan las páginas del informe
    if (error_parametros == true) {
        $('#mensaje-aviso-informe-fichero-cortes-tension').html(
            "<i class='icon-warning-sign color-rojo'></i> " + TLNT.Idiomas._("Parámetros del informe incorrectos") + " (" + descripcion_error_parametros.toLowerCase() + ")");
        elimina_elemento("pagina-informe-fichero-cortes-tension-cortes-tension");
        return;
    }
    ...
    function (data, status) {
        ...
        // Títulos de las páginas
        $('#titulo-informe-fichero-cortes-tension-cortes-tension').html(TLNT.Idiomas._("Cortes de tensión"));
