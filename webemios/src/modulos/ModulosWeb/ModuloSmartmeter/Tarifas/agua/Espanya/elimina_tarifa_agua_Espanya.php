<?php
    ...
    AuditoriaAcciones::comprobar_registrar_accion(ACCION_ELIMINAR_TARIFA_AGUA, $_POST);
    ...
    // Se comprueba si se está utilizando la tarifa de agua en algún sensor
    $consulta_sensores = "
        SELECT nombre
        FROM sensores
        WHERE
            ...
            AND (SUBSTRING_INDEX(SUBSTRING_INDEX(parametros_clase, '".SEPARADOR_PARAMETROS_COMPUESTOS."', ".(INDICE_PARAMETRO_CLASE_SENSOR_AGUA_ESPANYA_ID_TARIFA_AGUA + 1)."), '".SEPARADOR_PARAMETROS_COMPUESTOS."', -1)  = '".$bd_red->_($id_tarifa_agua)."')
        ORDER BY nombre ASC";
    ...
    else
    {
        ...
        if ($res_borrado == true)
        {
            // Se eliminan los conceptos adicionales de factura de la tarifa de agua
            $operacion_borrado_conceptos_adicionales_tarifa_agua = "
                DELETE
                FROM ".TABLA_CONCEPTOS_ADICIONALES_FACTURA_TARIFAS_AGUA_ESPANYA."
                WHERE
                    tarifa = '".$bd_red->_($id_tarifa_agua)."'";
            $res_borrado_conceptos_adicionales_tarifa_agua = $bd_red->ejecuta_operacion($operacion_borrado_conceptos_adicionales_tarifa_agua);
            if ($res_borrado_conceptos_adicionales_tarifa_agua == false)
            {
                throw new Exception("Error en la operación: '".$operacion_borrado_conceptos_adicionales_tarifa_agua."'");
            }

            // Acciones a realizar al eliminar la tarifa
            realiza_acciones_tarifa_eliminada(MEDICION_AGUA, $id_tarifa_agua);

            // Se añade la acción de usuario
            anyade_accion_usuario_eliminar_tarifa_agua_Espanya($fila_tarifa_agua);
    ...
    // Añade la acción de usuario de eliminación de la tarifa de agua
    function anyade_accion_usuario_eliminar_tarifa_agua_Espanya($fila)
    {
