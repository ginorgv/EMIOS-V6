//
// Funciones para el dibujado de los informes de facturas (SmartMeter) (gas - España)
//


// Dibujado del informe de simulación de factura
function dibuja_informe_smartmeter_simulador_factura_gas_Espanya(
    parametros,
    datos,
    elementos_informe,
    tipo_informe) {
    ...
    var msg_informacion = datos.msg_informacion;
    var tabla_coste_consumo = datos.tabla_coste_consumo;
    var tabla_consumo = datos.tabla_consumo;
    var tabla_termino_fijo = datos.tabla_termino_fijo;
    var tabla_capacidad_demandada = datos.tabla_capacidad_demandada;
    ...
    var id_contenedor_tabla_termino_fijo = parametros.id_contenedor_tabla_termino_fijo;
    var id_contenedor_tabla_capacidad_demandada = parametros.id_contenedor_tabla_capacidad_demandada;
    ...
    anyade_parametros_elementos_visibles_informe_smartmeter_facturas_gas_Espanya(
    ...
    var mostrar_tabla_termino_fijo = parametros.mostrar_tabla_termino_fijo;
    var mostrar_tabla_capacidad_demandada = datos.mostrar_tabla_capacidad_demandada;
    ...
    if (mostrar_tabla_termino_fijo == true) {
        muestra_elemento(id_contenedor_tabla_termino_fijo);
    }
    if (mostrar_tabla_capacidad_demandada == true) {
        muestra_elemento(id_contenedor_tabla_capacidad_demandada);
    }
    ...
    if (mostrar_tabla_termino_fijo == true) {
        $("#" + id_contenedor_tabla_termino_fijo).html(tabla_termino_fijo);
        if (anyadir_menus_contextuales == true) {
            var id_tabla_termino_fijo = anyade_id_tabla_id_contenedor_tabla(id_contenedor_tabla_termino_fijo);
            var info_menu_contextual = dame_info_menu_contextual_tablas_datos();
            anyade_menu_contextual(id_tabla_termino_fijo, info_menu_contextual, TLNT.Idiomas._('Término fijo'));
        }
    }
    // Tabla de capacidad demandada
    if (mostrar_tabla_capacidad_demandada == true) {
        $("#" + id_contenedor_tabla_capacidad_demandada).html(tabla_capacidad_demandada);
        if (anyadir_menus_contextuales == true) {
            var id_tabla_capacidad_demandada = anyade_id_tabla_id_contenedor_tabla(id_contenedor_tabla_capacidad_demandada);
            var info_menu_contextual = dame_info_menu_contextual_tablas_datos();
            anyade_menu_contextual(id_tabla_capacidad_demandada, info_menu_contextual, TLNT.Idiomas._('Capacidad demandada'));
        }
    }
...
// Añade a los parámetros los flags de elementos visibles
function anyade_parametros_elementos_visibles_informe_smartmeter_facturas_gas_Espanya(
    ...
    switch (tipo_informe_smartmeter_facturas) {
        case TIPO_INFORME_SMARTMETER_SIMULADOR_FACTURA: {
            ...
            var mostrar_tabla_termino_fijo = true;
            var mostrar_tabla_capacidad_demandada = true;
            ...
            if (elementos_informe != null) {
                if (elementos_informe.indexOf(ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_GAS_ESPANYA_TITULO_DATOS) == -1) {
                    mostrar_titulo_datos = false;
                }
                if (elementos_informe.indexOf(ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_GAS_ESPANYA_TABLA_DATOS) == -1) {
                    mostrar_tabla_datos = false;
                }
                if (elementos_informe.indexOf(ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_GAS_ESPANYA_TITULO_RESUMEN) == -1) {
                    mostrar_titulo_resumen = false;
                }
                if (elementos_informe.indexOf(ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_GAS_ESPANYA_TABLA_COSTE_CONSUMO) == -1) {
                    mostrar_tabla_coste_consumo = false;
                }
                if (elementos_informe.indexOf(ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_GAS_ESPANYA_TITULO_DETALLES) == -1) {
                    mostrar_titulo_detalles = false;
                }
                if (elementos_informe.indexOf(ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_GAS_ESPANYA_TABLA_CONSUMO) == -1) {
                    mostrar_tabla_consumo = false;
                }
                if (elementos_informe.indexOf(ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_GAS_ESPANYA_TABLA_TERMINO_FIJO) == -1) {
                    mostrar_tabla_termino_fijo = false;
                }
                if (elementos_informe.indexOf(ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_GAS_ESPANYA_TABLA_CAPACIDAD_DEMANDADA) == -1) {
                    mostrar_tabla_capacidad_demandada = false;
                }
                if (elementos_informe.indexOf(ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_GAS_ESPANYA_TABLA_OTROS_CONCEPTOS) == -1) {
                    mostrar_tabla_otros_conceptos = false;
                }
                if (elementos_informe.indexOf(ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_GAS_ESPANYA_GRAFICA_PORCENTAJES_COSTES_CONCEPTOS) == -1) {
                    mostrar_grafica_porcentajes_costes_conceptos = false;
                }
                if (elementos_informe.indexOf(ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_GAS_ESPANYA_TITULO_REPARTO_COSTES) == -1) {
                    mostrar_titulo_reparto_costes = false;
                }
                if (elementos_informe.indexOf(ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_GAS_ESPANYA_TABLA_REPARTO_COSTES) == -1) {
                    mostrar_tabla_reparto_costes = false;
                }
                if (elementos_informe.indexOf(ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_GAS_ESPANYA_GRAFICA_PORCENTAJES_REPARTO_COSTES) == -1) {
            ...
            parametros.mostrar_tabla_termino_fijo = mostrar_tabla_termino_fijo;
            parametros.mostrar_tabla_capacidad_demandada = mostrar_tabla_capacidad_demandada;
            ...
            var hay_elementos_visibles =
                ...
                (mostrar_tabla_termino_fijo == true) ||
...
function dibuja_elemento_plantilla_informe_smartmeter_simulador_factura_gas_Espanya(
    info_elemento,
    datos_elemento,
    ...
    var id_contenedor_tabla_termino_fijo = prefijo_elemento + "contenedor-tabla-termino-fijo-simulador-factura";
    var id_contenedor_tabla_capacidad_demandada = prefijo_elemento + "contenedor-tabla-capacidad-demandada-simulador-factura";
    ...
    var parametros = {
        ...
        id_contenedor_tabla_termino_fijo: id_contenedor_tabla_termino_fijo,
        id_contenedor_tabla_capacidad_demandada: id_contenedor_tabla_capacidad_demandada,
