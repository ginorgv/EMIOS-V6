<?php
    ...
    //
    // Funciones de información de facturas (agua - España)
    //


    // Devuelve la información de simulación de factura de un sensor y tarifa
    function dame_simulacion_factura_sensor_tarifa_agua_Espanya($parametros)
    {
        $idiomas = new Idiomas();
        ...
        // Se recupera el identificador de tarifa de agua si no hay tarifa de agua seleccionada
        ...
        if ($id_tarifa == ID_NINGUNO)
        {
            $mensaje_error = $idiomas->_("El sensor no tiene tarifa de agua asignada");
            $resultado = array(
                "res" => "ERROR",
                "msg" => $mensaje_error);
            return ($resultado);
        ...
        $tipo_limites_consumo_tramos_tarifa_agua = $fila_tarifa_agua["tipo_limites_consumo_tramos"];
        ...
        else
        {
            ...
            $cups = $parametros_clase_sensor[INDICE_PARAMETRO_CLASE_SENSOR_AGUA_ESPANYA_CUPS];
            ...
            $datos_nombre_tarifa = array(
                $idiomas->_("Tarifa"),
                $fila_tarifa_agua["nombre"]." (".TarifaAgua_Espanya::dame_descripcion_tipo_tarifa_agua($tipo_tarifa_agua).")");
            ...
            // Consumos y costes de consumo (por tramos)
            $numero_dias_consumo_agua = $resultado_funcion_externa["numero_dias_consumo_agua"];
            $cadena_hora_inicio_consumos_agua_funciones_utc = $resultado_funcion_externa["hora_inicio_consumos_agua"];
            $cadena_hora_fin_consumos_agua_funciones_utc = $resultado_funcion_externa["hora_fin_consumos_agua"];
            $limites_consumo_tramos = $resultado_funcion_externa["limites_consumo_tramos"];
            $precios_consumo_tramos = $resultado_funcion_externa["precios_consumo_tramos"];
            $consumos_tramos = $resultado_funcion_externa["consumos_tramos"];
            $costes_tramos = $resultado_funcion_externa["costes_tramos"];
            ...
            $params_tabla_consumo = array(
                "numero_columnas" => NUMERO_COLUMNAS_TABLA_CONSUMO_SENSOR_TARIFA_AGUA_ESPANYA,
                "anchuras_columnas" => unserialize(ANCHURAS_COLUMNAS_TABLA_CONSUMO_SENSOR_TARIFA_AGUA_ESPANYA),
                "generar_valores_xml" => true
            );
            $tabla_consumo = new TablaDatos(
                "tabla-consumo-simulador-factura-agua",
                $idiomas->_("Consumo"),
                TIPO_TABLA_DATOS_LISTA,
            ...
            for ($i = 0; $i < $numero_tramos; $i++)
            {
                if ($i < ($numero_tramos - 1))
                {
                    ...
                    switch ($tipo_limites_consumo_tramos_tarifa_agua)
                    {
                ...
                $datos_fila_consumo = array(
                    ...
                    formatea_numero($consumos_tramos[$i], 2)." ".$idiomas->_("m3")."<font class='color-gris-muy-claro'> x </font>".
                        formatea_numero($precios_consumo_tramos[$i], 6)." ".$idiomas->_("€")."/".$idiomas->_("m3"),
                    formatea_numero($costes_tramos[$i], 2, false)." ".$unidad_medida_coste);
            ...
            $consumos_tramos_conceptos_adicionales = $resultado_funcion_externa["consumos_tramos_conceptos_adicionales"];
            $costes_consumo_tramos_conceptos_adicionales = $resultado_funcion_externa["costes_consumo_tramos_conceptos_adicionales"];
            $impuestos_conceptos_adicionales = $resultado_funcion_externa["impuestos_conceptos_adicionales"];
            $iva_igic_consumo = $resultado_funcion_externa["iva_igic_consumo"];
            $iva_igic_alquiler_contador = $resultado_funcion_externa["iva_igic_alquiler_contador"];
            $coste_alquiler_contador = $resultado_funcion_externa["coste_alquiler_contador"];
            $coste_iva_igic = $resultado_funcion_externa["coste_iva_igic"];
            ...
            $params_tabla_otros_conceptos = array(
                "numero_columnas" => NUMERO_COLUMNAS_TABLA_OTROS_CONCEPTOS_SENSOR_TARIFA_AGUA_ESPANYA,
                "anchuras_columnas" => unserialize(ANCHURAS_COLUMNAS_TABLA_OTROS_CONCEPTOS_SENSOR_TARIFA_AGUA_ESPANYA),
                "generar_valores_xml" => true
            );
            $tabla_otros_conceptos = new TablaDatos(
                "tabla-otros-conceptos-simulador-factura-agua",
            ...
            if ($alquiler_contador > 0)
            {
                ...
                switch ($tipo_alquiler_contador)
                {
                    ...
                    {
                        $texto_calculo_alquiler_contador = formatea_numero($alquiler_contador, 2)." ".$unidad_medida_coste."/".$idiomas->_("día")."<font class='color-gris-muy-claro'> x </font>".
                            $numero_dias." ".$idiomas->_("días");
                        break;
                    }
                    case TIPO_ALQUILER_CONTADOR_FIJO:
                    {
                        $texto_calculo_alquiler_contador = formatea_numero($alquiler_contador, 2)." ".$unidad_medida_coste;
                        break;
                    }
                }
            ...
            $info_conceptos_adicionales_factura = dame_info_conceptos_adicionales_factura_tarifa(MEDICION_AGUA, $id_tarifa);
            for ($i = 0; $i < count($info_conceptos_adicionales_factura); $i++)
            {
                ...
                $consumos_tramos_concepto_adicional_factura = $consumos_tramos_conceptos_adicionales[$i];
                $costes_consumo_tramos_concepto_adicional_factura = $costes_consumo_tramos_conceptos_adicionales[$i];
                ...
                switch ($info_concepto_adicional_factura["tipo"])
                {
                    ...
                    case TIPO_CONCEPTO_ADICIONAL_FACTURA_TARIFA_CONSUMO_DIARIO:
                    {
                        ...
                        for ($j = 0; $j < count($consumos_tramos_concepto_adicional_factura); $j++)
                        {
                            if ($j > 0)
                            {
                                $cadena_calculo_coste_concepto_adicional .= "<font class='color-gris-muy-claro'> + </font>";
                            }
                            $cadena_calculo_coste_concepto_adicional .= formatea_numero($consumos_tramos_concepto_adicional_factura[$j], 2, false)." ".$idiomas->_("m3").
                                "<font class='color-gris-muy-claro'> x </font>".
                                formatea_numero($costes_consumo_tramos_concepto_adicional_factura[$j], 2, false)." ".$idiomas->_("€")."/".$idiomas->_("m3");
                        }
                        break;
                    }
                }
            ...
            // Base imponible
            $base_imponible = $coste_consumo + $coste_alquiler_contador;
            for ($i = 0; $i < count($info_conceptos_adicionales_factura); $i++)
            {
            ...
            if ($coste_iva_igic > 0)
            {
                // Cadena de cálculo de IVA / IGIC (total)
                $cadena_calculo_iva_igic_consumo_otros_conceptos =
                    formatea_numero($coste_consumo, 2, false)." ".$unidad_medida_coste.
                    "<font class='color-gris-muy-claro'> x </font>".
                    formatea_numero($iva_igic_consumo, 2, false)." "."%";
                if ($coste_alquiler_contador > 0)
                {
                    $cadena_calculo_iva_igic_consumo_otros_conceptos .=
                        "<font class='color-gris-muy-claro'> + </font>".
                        formatea_numero($coste_alquiler_contador, 2, false)." ".$unidad_medida_coste.
                        "<font class='color-gris-muy-claro'> x </font>".
                        formatea_numero($iva_igic_alquiler_contador, 2, false)." "."%";
                }
                for ($i = 0; $i < count($info_conceptos_adicionales_factura); $i++)
                {
                    $cadena_calculo_iva_igic_consumo_otros_conceptos .=
                        "<font class='color-gris-muy-claro'> + </font>".
                        formatea_numero($costes_conceptos_adicionales[$i], 2, false)." ".$unidad_medida_coste.
                        "<font class='color-gris-muy-claro'> x </font>".
                        formatea_numero($impuestos_conceptos_adicionales[$i], 2, false)." "."%";
                }

                // Descripción de IVA / IGIC
                $descripcion_iva_igic = NULL;
                if ($tipo_tarifa_canarias == false)
                {
                    $descripcion_iva_igic = $idiomas->_("IVA");
                }
                else
                {
                    $descripcion_iva_igic = $idiomas->_("IGIC");
                }

                // Se crean los datos para la fila de la tabla y se añade
                $datos_fila_iva_igic = array(
                    $descripcion_iva_igic,
                    $cadena_calculo_iva_igic_consumo_otros_conceptos,
                    formatea_numero($coste_iva_igic, 2, false)." ".$unidad_medida_coste);
                $tabla_otros_conceptos->anyade_fila("fila-iva-igic", $datos_fila_iva_igic);
            }

            // Coste total de otros conceptos
            // Se cambia el texto para más claridad y que no se confunda con el coste total de la factura
            $pie_tabla_otros_conceptos = $idiomas->_("Total otros conceptos").": ".formatea_numero($coste_total_otros_conceptos, 2, false)." ".$unidad_medida_coste;
            ...
            $params_tabla_coste_consumo = array(
                "numero_columnas" => NUMERO_COLUMNAS_TABLA_COSTE_CONSUMO_SENSOR_TARIFA_AGUA_ESPANYA,
                "anchuras_columnas" => unserialize(ANCHURAS_COLUMNAS_TABLA_COSTE_CONSUMO_SENSOR_TARIFA_AGUA_ESPANYA),
                "generar_valores_xml" => true
            );
            $tabla_coste_consumo = new TablaDatos(
                "tabla-coste-consumo-simulador-factura-agua",
            ...
            $datos_fila_coste_consumo = array(
                ...
                formatea_numero($consumo, 2)." ".$idiomas->_("m3"),
                formatea_numero($coste_diario, 2, false)." ".$unidad_medida_coste."/".$idiomas->_("día"),
                formatea_numero($consumo_diario, 2)." ".$idiomas->_("m3")."/".$idiomas->_("día"));
            ...
            // Aviso de número de días de factura menor que número de días de consumo de agua
            if ($numero_dias > $numero_dias_consumo_agua)
            {
                // Conversión de fechas
                $cadena_hora_inicio_consumos_agua_funciones_local = cambia_zona_horaria_cadena_fecha_hora($cadena_hora_inicio_consumos_agua_funciones_utc, FORMATO_FECHA_HORA_FUNCIONES, ZONA_HORARIA_UTC, $zona_horaria);
                $cadena_hora_fin_consumos_agua_funciones_local = cambia_zona_horaria_cadena_fecha_hora($cadena_hora_fin_consumos_agua_funciones_utc, FORMATO_FECHA_HORA_FUNCIONES, ZONA_HORARIA_UTC, $zona_horaria);
                $cadena_hora_inicio_consumos_agua_local_local = convierte_formato_fecha($cadena_hora_inicio_consumos_agua_funciones_local, FORMATO_FECHA_HORA_FUNCIONES, $_SESSION["formato_fecha_hora_local"]);
                $cadena_hora_fin_consumos_agua_local_local = convierte_formato_fecha($cadena_hora_fin_consumos_agua_funciones_local, FORMATO_FECHA_HORA_FUNCIONES, $_SESSION["formato_fecha_hora_local"]);

                // Se añade el aviso
                $aviso = $idiomas->_("El número de horas de la factura es mayor que el número de horas de consumo de agua")."\n(".
                    $idiomas->_("hora de inicio de consumo").": ".$cadena_hora_inicio_consumos_agua_local_local.", ".
                    $idiomas->_("hora de fin de consumo").": ".$cadena_hora_fin_consumos_agua_local_local.")";
    ...
    function dame_coste_concepto_simulacion_factura_sensor_agua_Espanya($parametros)
    {
        // Parámetros
        $id_sensor = $parametros["id_sensor"];
        $nombre_sensor = $parametros["nombre_sensor"];
        ...
        else
        {
            ...
            {
                // Se recupera el coste del concepto especificado de la factura de agua
                $coste_concepto_factura = NULL;
                switch ($concepto_factura)
                {
                    case CONCEPTO_FACTURA_AGUA_TOTAL_ESPANYA:
                    {
                        $coste_concepto_factura = $resultado_funcion_externa["coste_total"];
                        break;
                    }
                    case CONCEPTO_FACTURA_AGUA_CONSUMO_ESPANYA:
                    {
                        $coste_concepto_factura = $resultado_funcion_externa["coste_consumo"];
                        break;
                    }
                    case CONCEPTO_FACTURA_AGUA_OTROS_CONCEPTOS_ESPANYA:
                    {
                    ...
                    default:
                    {
                        throw new Exception("Concepto de factura de agua desconocido: '".$concepto_factura."'");
                    }
                }
    ...
    function dame_elementos_informe_smartmeter_simulador_factura_agua_Espanya()
    {
        $elementos_informe = array();
        array_push($elementos_informe, ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_TITULO_DATOS);
        array_push($elementos_informe, ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_TABLA_DATOS);
        array_push($elementos_informe, ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_TITULO_RESUMEN);
        array_push($elementos_informe, ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_TABLA_COSTE_CONSUMO);
        array_push($elementos_informe, ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_TITULO_DETALLES);
        array_push($elementos_informe, ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_TABLA_CONSUMO);
        array_push($elementos_informe, ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_TABLA_OTROS_CONCEPTOS);
        array_push($elementos_informe, ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_GRAFICA_PORCENTAJES_COSTES_CONCEPTOS);
        array_push($elementos_informe, ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_TITULO_REPARTO_COSTES);
        array_push($elementos_informe, ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_TABLA_REPARTO_COSTES);
        array_push($elementos_informe, ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_GRAFICA_PORCENTAJES_REPARTO_COSTES);
        return ($elementos_informe);
    }


    function dame_descripcion_elemento_informe_simulacion_factura_agua_Espanya($elemento_informe)
    {
        switch ($elemento_informe)
        {
            case ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_TITULO_DATOS:
            {
                $descripcion = "Título de datos de factura";
                break;
            }
            case ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_TABLA_DATOS:
            {
                $descripcion = "Tabla de datos de factura";
                break;
            }
            case ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_TITULO_RESUMEN:
            {
                $descripcion = "Título de resumen de factura";
                break;
            }
            case ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_TABLA_COSTE_CONSUMO:
            {
                $descripcion = "Tabla de coste y consumo";
                break;
            }
            case ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_TITULO_DETALLES:
            {
                $descripcion = "Título de detalles de factura";
                break;
            }
            case ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_TABLA_CONSUMO:
            {
                $descripcion = "Tabla de consumo";
                break;
            }
            case ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_TABLA_OTROS_CONCEPTOS:
            {
                $descripcion = "Tabla de otros conceptos";
                break;
            }
            case ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_GRAFICA_PORCENTAJES_COSTES_CONCEPTOS:
            {
                $descripcion = "Gráfica de porcentajes de costes por concepto";
                break;
            }
            case ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_TITULO_REPARTO_COSTES:
            {
                $descripcion = "Título de reparto de costes";
                break;
            }
            case ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_TABLA_REPARTO_COSTES:
            {
                $descripcion = "Tabla de reparto de costes";
                break;
            }
            case ELEMENTO_INFORME_SMARTMETER_SIMULADOR_FACTURA_AGUA_ESPANYA_GRAFICA_PORCENTAJES_REPARTO_COSTES:
            {
    ...
    function dame_html_informe_tipo_smartmeter_simulador_factura_agua_Espanya($tipo_informe)
    {
        $idiomas = new Idiomas();
    ...
    function dame_html_elemento_plantilla_informe_tipo_smartmeter_simulador_factura_agua_Espanya(
        $numero_elemento,
        $nombre_elemento,
    ...
    function dame_datos_elemento_plantilla_informe_tipo_smartmeter_simulador_factura_agua_Espanya(
        $numero_elemento,
        $parametros_tipo_elemento,
        $parametros_informe)
    {
        ...
        $datos_elemento = dame_simulacion_factura_sensor_tarifa_agua_Espanya($parametros_informe);
