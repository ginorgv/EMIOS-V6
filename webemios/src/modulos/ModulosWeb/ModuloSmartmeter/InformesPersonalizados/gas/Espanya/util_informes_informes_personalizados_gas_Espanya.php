<?php
    ...
    include_once($_SESSION["directorio"].'/src/modulos/ModulosWeb/ModuloSmartmeter/gas/util_gas.php');
    ...
    //
    // Funciones de información de informes personalizados (gas - España)
    //


    // Devuelve el estudio general de un sensor
    function dame_estudio_general_sensor_gas_Espanya($parametros)
    {
        ...
        // Nota: Se añaden 6 horas a las horas de inicio y de fin
        ...
        $fecha_hora_inicio_local->modify('+6 hours');
        ...
        $fecha_hora_fin_local->modify('+6 hours');
        ...
        // Portada
        if (in_array(APARTADO_INFORME_ESTUDIO_GENERAL_PORTADA_GAS_ESPANYA, $apartados) == true)
        {
            // Nombre de red
            $consulta_nombre_red = "
                SELECT nombre
        ...
        if (in_array(APARTADO_INFORME_ESTUDIO_GENERAL_INSTALACION_GAS_ESPANYA, $apartados) == true)
        {
            $datos_instalacion = dame_datos_instalacion_sensor(MEDICION_GAS, $id_sensor, $id_tarifa);
        }

        // Análisis de consumo
        if (in_array(APARTADO_INFORME_ESTUDIO_GENERAL_ANALISIS_CONSUMO_GAS_ESPANYA, $apartados) == true)
        {
            // Información de gas (consumo por días)
            $parametros_informacion_sensor_gas_consumo_dias = array(
                "id_ratio" => $id_ratio,
            ...
            $res_informacion_sensor_gas_consumo_dias = dame_informacion_sensor_gas(
                $parametros_informacion_sensor_gas_consumo_dias,
                $filas_valores_sensor_dias);

            // Datos del apartado (gráfica de consumos)
            $datos_analisis_consumo["hay_datos_grafica_consumos"] = $res_informacion_sensor_gas_consumo_dias["hay_datos"];
            if ($datos_analisis_consumo["hay_datos_grafica_consumos"] == true)
            {
                $datos_analisis_consumo["max_consumo"] = $res_informacion_sensor_gas_consumo_dias["max_valor"];
                $datos_analisis_consumo["etiquetas_grafica_consumos"] = $res_informacion_sensor_gas_consumo_dias["etiquetas_graficas"];
                $datos_analisis_consumo["grafica_consumos"] = $res_informacion_sensor_gas_consumo_dias["grafica_valores"];
            }

            // Información de gas (consumo por horas)
            $parametros_informacion_sensor_gas_consumo_horas = array(
                "id_ratio" => $id_ratio,
                "clase_sensor" => CLASE_SENSOR_GAS,
                "id_sensor" => $id_sensor,
                "nombre_sensor" => $nombre_sensor,
                ...
                "minutos_desfase_utc" => $minutos_desfase_utc,
                "tipo_informe" => $tipo_informe);
            $res_informacion_sensor_gas_consumo_horas = dame_informacion_sensor_gas(
                $parametros_informacion_sensor_gas_consumo_horas,
                $filas_valores_sensor_horas);

            // Datos del apartado (mapa de calor de consumos)
            $datos_analisis_consumo["hay_datos_mapa_calor_consumos"] = $res_informacion_sensor_gas_consumo_horas["hay_datos"];
            if ($datos_analisis_consumo["hay_datos_mapa_calor_consumos"] == true)
            {
                $datos_analisis_consumo["colores_mapa_calor_consumos"] = $res_informacion_sensor_gas_consumo_horas["colores_mapa_calor_valores"];
                $datos_analisis_consumo["dias_mapa_calor_consumos"] = $res_informacion_sensor_gas_consumo_horas["dias_mapa_calor_valores"];
                $datos_analisis_consumo["datos_mapa_calor_consumos"] = $res_informacion_sensor_gas_consumo_horas["datos_mapa_calor_valores"];
                $datos_analisis_consumo["unidad_medida_consumo"] = $res_informacion_sensor_gas_consumo_horas["unidad_medida"];
            }
        }

        // Análisis de coste
        if (in_array(APARTADO_INFORME_ESTUDIO_GENERAL_ANALISIS_COSTE_GAS_ESPANYA, $apartados) == true)
        {
            // Información de gas (coste por días)
            $parametros_informacion_sensor_gas_coste_dias = array(
                "id_ratio" => $id_ratio,
                "clase_sensor" => CLASE_SENSOR_GAS,
                "id_sensor" => $id_sensor,
                "nombre_sensor" => $nombre_sensor,
                ...
                "minutos_desfase_utc" => $minutos_desfase_utc,
                "tipo_informe" => $tipo_informe);
            $res_informacion_sensor_gas_coste_dias = dame_informacion_sensor_gas(
                $parametros_informacion_sensor_gas_coste_dias,
                $filas_valores_sensor_dias);

            // Datos del apartado (gráfica de costes)
            $datos_analisis_coste["hay_datos_grafica_costes"] = $res_informacion_sensor_gas_coste_dias["hay_datos"];
            if ($datos_analisis_coste["hay_datos_grafica_costes"] == true)
            {
                $datos_analisis_coste["max_coste"] = $res_informacion_sensor_gas_coste_dias["max_valor"];
                $datos_analisis_coste["etiquetas_grafica_costes"] = $res_informacion_sensor_gas_coste_dias["etiquetas_graficas"];
                $datos_analisis_coste["grafica_costes"] = $res_informacion_sensor_gas_coste_dias["grafica_valores"];
            }

            // Información de gas (coste por horas)
            $parametros_informacion_sensor_gas_coste_horas = array(
                "id_ratio" => $id_ratio,
                "clase_sensor" => CLASE_SENSOR_GAS,
                "id_sensor" => $id_sensor,
                "nombre_sensor" => $nombre_sensor,
                ...
                "minutos_desfase_utc" => $minutos_desfase_utc,
                "tipo_informe" => $tipo_informe);
            $res_informacion_sensor_gas_coste_horas = dame_informacion_sensor_gas(
                $parametros_informacion_sensor_gas_coste_horas,
                $filas_valores_sensor_horas);

            // Datos del apartado (mapa de calor de costes)
            $datos_analisis_coste["hay_datos_mapa_calor_costes"] = $res_informacion_sensor_gas_coste_horas["hay_datos"];
            if ($datos_analisis_coste["hay_datos_mapa_calor_costes"] == true)
            {
                $datos_analisis_coste["colores_mapa_calor_costes"] = $res_informacion_sensor_gas_coste_horas["colores_mapa_calor_valores"];
                $datos_analisis_coste["dias_mapa_calor_costes"] = $res_informacion_sensor_gas_coste_horas["dias_mapa_calor_valores"];
                $datos_analisis_coste["datos_mapa_calor_costes"] = $res_informacion_sensor_gas_coste_horas["datos_mapa_calor_valores"];
                $datos_analisis_coste["unidad_medida_coste"] = $res_informacion_sensor_gas_coste_horas["unidad_medida"];
            }
        }

        // Análisis de consumo y coste
        if ((in_array(APARTADO_INFORME_ESTUDIO_GENERAL_ANALISIS_CONSUMO_GAS_ESPANYA, $apartados) == true) ||
            (in_array(APARTADO_INFORME_ESTUDIO_GENERAL_ANALISIS_COSTE_GAS_ESPANYA, $apartados) == true))
        {
        ...
        // Excesos de caudal
        if (in_array(APARTADO_INFORME_ESTUDIO_GENERAL_EXCESOS_CAUDAL_GAS_ESPANYA, $apartados) == true)
        {
            ...
            if ($recuperar_datos_excesos_potencia == true)
            {
                ...
                $recuperar_datos_excesos_caudal = ($tipo_calculo_coste_termino_fijo_sensor != TIPO_CALCULO_COSTE_TERMINO_FIJO_SIN_EXCESOS);
            }

            // Datos del apartado (excesos de caudal)
            $datos_excesos_caudal["hay_datos_excesos_caudal"] = $recuperar_datos_excesos_caudal;
            if ($datos_excesos_caudal["hay_datos_excesos_caudal"] == true)
            {
                // Nota: No hay mapa de calor de sobrecaudales (porque es diario, no horario)

                // Información de sobrecaudales (por horas)
                $parametros_sobrecaudales_sensor = array(
                    "id_sensor" => $id_sensor,
                    "nombre_sensor" => $nombre_sensor,
                    "fecha_hora_inicio" => $cadena_fecha_hora_inicio_local_local,
                    "fecha_hora_fin" => $cadena_fecha_hora_fin_local_local,
                    "minutos_desfase_utc" => $minutos_desfase_utc);
                $res_sobrecaudales_sensor = dame_sobrecaudales_sensor_gas_Espanya(
                    $parametros_sobrecaudales_sensor);

                // Datos del apartado (gráfica de caudales y sobrecaudales y tabla de sobrecaudales)
                $datos_excesos_caudal["hay_datos_grafica_caudales_sobrecaudales_tabla_sobrecaudales"] = $res_sobrecaudales_sensor["hay_datos"];
                if ($datos_excesos_caudal["hay_datos_grafica_caudales_sobrecaudales_tabla_sobrecaudales"] == true)
                {
                    $datos_excesos_caudal["grafica_caudales_sobrecaudales"] = $res_sobrecaudales_sensor["grafica_caudales_sobrecaudales"];
                    $datos_excesos_caudal["max_caudal"] = $res_sobrecaudales_sensor["max_caudal"];
                    $datos_excesos_caudal["caudal_diario_contratado"] = $res_sobrecaudales_sensor["caudal_diario_contratado"];
                    $datos_excesos_caudal["tabla_sobrecaudales"] = $res_sobrecaudales_sensor["tabla_sobrecaudales"];
                }
            }
        }

        // Simulación de factura
        if (in_array(APARTADO_INFORME_ESTUDIO_GENERAL_SIMULACION_FACTURA_GAS_ESPANYA, $apartados) == true)
        {
            ...
            if ($datos_simulacion_factura["hay_datos_simulacion_factura"] == true)
            {
                ...
                $parametros_simulacion_factura_sensor_tarifa = array(
                    ...
                    "fecha_hora_inicio" => $cadena_fecha_hora_inicio_local_local,
                    "fecha_hora_fin" => $cadena_fecha_hora_fin_local_local);
                $res_simulacion_factura_sensor_tarifa = dame_simulacion_factura_sensor_tarifa_gas_Espanya(
    ...
    function dame_controles_textos_estudio_general_gas_Espanya()
    {
        $idiomas = new Idiomas();
    ...
    function dame_lista_apartados_estudio_general_gas_Espanya()
    {
        $lista_apartados .= "<option value='".APARTADO_INFORME_ESTUDIO_GENERAL_PORTADA_GAS_ESPANYA."' sort_id='01'>".dame_descripcion_apartado_estudio_general_gas_Espanya(APARTADO_INFORME_ESTUDIO_GENERAL_PORTADA_GAS_ESPANYA)."</option>";
        $lista_apartados .= "<option value='".APARTADO_INFORME_ESTUDIO_GENERAL_INTRODUCCION_GAS_ESPANYA."' sort_id='02'>".dame_descripcion_apartado_estudio_general_gas_Espanya(APARTADO_INFORME_ESTUDIO_GENERAL_INTRODUCCION_GAS_ESPANYA)."</option>";
        $lista_apartados .= "<option value='".APARTADO_INFORME_ESTUDIO_GENERAL_INSTALACION_GAS_ESPANYA."' sort_id='03'>".dame_descripcion_apartado_estudio_general_gas_Espanya(APARTADO_INFORME_ESTUDIO_GENERAL_INSTALACION_GAS_ESPANYA)."</option>";
        $lista_apartados .= "<option value='".APARTADO_INFORME_ESTUDIO_GENERAL_ANALISIS_CONSUMO_GAS_ESPANYA."' sort_id='04'>".dame_descripcion_apartado_estudio_general_gas_Espanya(APARTADO_INFORME_ESTUDIO_GENERAL_ANALISIS_CONSUMO_GAS_ESPANYA)."</option>";
        $lista_apartados .= "<option value='".APARTADO_INFORME_ESTUDIO_GENERAL_ANALISIS_COSTE_GAS_ESPANYA."' sort_id='06'>".dame_descripcion_apartado_estudio_general_gas_Espanya(APARTADO_INFORME_ESTUDIO_GENERAL_ANALISIS_COSTE_GAS_ESPANYA)."</option>";
        $lista_apartados .= "<option value='".APARTADO_INFORME_ESTUDIO_GENERAL_EXCESOS_CAUDAL_GAS_ESPANYA."' sort_id='07'>".dame_descripcion_apartado_estudio_general_gas_Espanya(APARTADO_INFORME_ESTUDIO_GENERAL_EXCESOS_CAUDAL_GAS_ESPANYA)."</option>";
        $lista_apartados .= "<option value='".APARTADO_INFORME_ESTUDIO_GENERAL_SIMULACION_FACTURA_GAS_ESPANYA."' sort_id='08'>".dame_descripcion_apartado_estudio_general_gas_Espanya(APARTADO_INFORME_ESTUDIO_GENERAL_SIMULACION_FACTURA_GAS_ESPANYA)."</option>";
        $lista_apartados .= "<option value='".APARTADO_INFORME_ESTUDIO_GENERAL_CONCLUSIONES_GAS_ESPANYA."' sort_id='09'>".dame_descripcion_apartado_estudio_general_gas_Espanya(APARTADO_INFORME_ESTUDIO_GENERAL_CONCLUSIONES_GAS_ESPANYA)."</option>";
    ...
    function dame_descripcion_apartado_estudio_general_gas_Espanya($apartado)
    {
        switch ($apartado)
        {
            case APARTADO_INFORME_ESTUDIO_GENERAL_PORTADA_GAS_ESPANYA:
            {
                $descripcion = "Portada";
                break;
            }
            case APARTADO_INFORME_ESTUDIO_GENERAL_INTRODUCCION_GAS_ESPANYA:
            {
                $descripcion = "Introducción";
                break;
            }
            case APARTADO_INFORME_ESTUDIO_GENERAL_INSTALACION_GAS_ESPANYA:
            {
                $descripcion = "Instalación";
                break;
            }
            case APARTADO_INFORME_ESTUDIO_GENERAL_ANALISIS_CONSUMO_GAS_ESPANYA:
            {
                $descripcion = "Análisis de consumo";
                break;
            }
            case APARTADO_INFORME_ESTUDIO_GENERAL_ANALISIS_COSTE_GAS_ESPANYA:
            {
                $descripcion = "Análisis de coste";
                break;
            }
            case APARTADO_INFORME_ESTUDIO_GENERAL_EXCESOS_CAUDAL_GAS_ESPANYA:
            {
                $descripcion = "Excesos de caudal";
                break;
            }
            case APARTADO_INFORME_ESTUDIO_GENERAL_SIMULACION_FACTURA_GAS_ESPANYA:
            {
                $descripcion = "Simulación de factura";
                break;
            }
            case APARTADO_INFORME_ESTUDIO_GENERAL_CONCLUSIONES_GAS_ESPANYA:
            {
    ...
    function dame_html_informe_tipo_smartmeter_estudio_general_gas_Espanya($tipo_informe)
    {
        $idiomas = new Idiomas();
        ...
        switch ($tipo_informe)
        {
                ...
                $html_informe = "
                    ...
                    <div id='informe-smartmeter-estudio-general' hidden>
                        ...
                        <div id='apartado-instalacion-estudio-general' class='apartado-informe'>
                            ...
                            <div class='tabla-datos100' id='contenedor-tabla-parametros-tarifa-gas-instalacion-estudio-general'></div>
                            <div class='contenedor-texto-informe'>
                        ...
                        <div id='apartado-excesos-caudal-estudio-general' class='apartado-informe'>
                            <div class='contenedor-titulo-informe'>
                                <div id='titulo-sobrecaudal-estudio-general' class='titulo-informe'>".$idiomas->_("Excesos de caudal")."</div>
                            </div>
                            <div class='grafica100' id='grafica-caudales-sobrecaudales-excesos-caudal-estudio-general'></div>
                            <div class='tabla-datos100' id='contenedor-tabla-sobrecaudales-excesos-caudal-estudio-general'></div>
                            <div class='contenedor-texto-informe'>
                                <span class='titulo-texto-informe'>".$idiomas->_('Notas').": "."</span>".
                                "<span class='titulo-campo-administracion contador-caracteres-textarea' numero_maximo_caracteres='".NUMERO_MAXIMO_CARACTERES_NOTAS."'>".
                                    "(0"." / ".NUMERO_MAXIMO_CARACTERES_NOTAS.")"."</span><br/>
                                <textarea class='area-texto-informe' id='notas-excesos-caudal-estudio-general' rows='1'></textarea>
                            </div>
                        </div>
            ...
            case TIPO_INFORME_FICHERO:
            {
                ...
                $html_informe .= "
                        ...
                        <br/>
                        <div class='tabla-datos100-informe-fichero separacion-superior-elementos-informe-fichero' id='contenedor-tabla-parametros-tarifa-gas-instalacion-estudio-general'></div>
                ...
                // Página de análisis de coste
                $html_informe .= "
                    <div class='pagina-informe-fichero' id='pagina-informe-fichero-estudio-general-analisis-coste'>";
                $html_informe .= dame_html_cabecera_informe_fichero_smartmeter_informes_personalizados(TIPO_INFORME_SMARTMETER_ESTUDIO_GENERAL);
                $html_informe .= "
                ...
                $html_informe .= "
                    <div class='pagina-informe-fichero' id='pagina-informe-fichero-estudio-general-excesos-caudal'>";
                $html_informe .= dame_html_cabecera_informe_fichero_smartmeter_informes_personalizados(TIPO_INFORME_SMARTMETER_ESTUDIO_GENERAL);
                $html_informe .= "
                        <div class='titulo-informe-fichero' id='titulo-informe-fichero-estudio-general-excesos-caudal'></div>
                        <div class='grafica100-informe-fichero separacion-superior-elementos-informe-fichero' id='grafica-caudales-sobrecaudales-excesos-caudal-estudio-general'></div>
                        <div class='tabla-datos100-informe-fichero separacion-superior-elementos-informe-fichero' id='contenedor-tabla-sobrecaudales-excesos-caudal-estudio-general'></div>
                        <div class='texto-informe-informe-fichero separacion-superior-elementos-informe-fichero' id='notas-excesos-caudal-estudio-general'></div>
                        <div class='fin-pagina-informe-fichero'></div>
                    </div>";
