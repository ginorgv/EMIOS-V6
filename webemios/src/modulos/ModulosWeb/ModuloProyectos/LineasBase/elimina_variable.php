<?php
    ...
    AuditoriaAcciones::comprobar_registrar_accion(ACCION_ELIMINAR_VARIABLE_LINEA_BASE, $_POST);
    ...
    // Se recupera la fila de la variable de la línea base
	$fila_variable_linea_base = dame_fila_variable_linea_base($id_variable);

    // Se elimina la variable de la línea base
	$operacion_borrado = "
        DELETE
    ...
    if ($res_borrado == true)
    {
        ...
        anyade_accion_usuario_eliminar_variable_linea_base($fila_variable_linea_base);

        $res = "OK";
        $msg = $idiomas->_("Variable eliminada correctamente");
    }
    else
    {
    ...
    // Añade la acción de usuario de eliminación de la variable de la línea base
    function anyade_accion_usuario_eliminar_variable_linea_base($fila)
    {
        // Nombre de la línea base
        $nombre_linea_base = dame_nombre_linea_base($fila["linea_base"]);

        // Tipo y objeto de la acción
        $tipo_accion_usuario = TIPO_ACCION_USUARIO_ELIMINAR_VARIABLE_LINEA_BASE;
