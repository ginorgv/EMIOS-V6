...
(function($)
{
	...
	function internalSort(a, b) {
        var compA = null;
        var compB = null;
		if ($(a).attr("sort_id") === undefined) {
            compA = $(a).text().toUpperCase();
            compB = $(b).text().toUpperCase();
        }
        else {
            compA = $(a).attr("sort_id");
            compB = $(b).attr("sort_id");
        }
	...
	var methods = {
		init : function(options) {
			...
			return this.each(function () {
                ...
                var options_class = "";
                if ((o.labelsx || leftSearch != false) || (o.labeldx || rightSearch != false)) {
                    options_class = "ms2side__options ms2side__options__search";
                }
                else {
                    options_class = "ms2side__options";
                }
				var htmlToAdd =
				...
				if (o.optGroupSearch != false) {
                    ...
                    // Nota: Al pasar a jquery 3.3.1, se ha cambiado la función 'size()' por el atributo 'length' (si no no funcionaba)
				...
				var	fto = function() {
                    ...
                    // Nota: Búsqueda múltiple
                    var filtro_busqueda = searchV;
                    var busqueda_multiple = false;
                    var tipo_busqueda_multiple = "";
                    var textos_busqueda = [];
                    var longitud_cadena_busqueda = filtro_busqueda.length;
                    if (longitud_cadena_busqueda > 0) {
                        if ((filtro_busqueda[0] == '+') || (filtro_busqueda[0] == '*')) {
                            tipo_busqueda_multiple = filtro_busqueda[0];
                            filtro_busqueda = filtro_busqueda.replace(/\s+/, "");
                            longitud_cadena_busqueda = filtro_busqueda.length;
                            if ((filtro_busqueda[1] == "{") && (filtro_busqueda[longitud_cadena_busqueda - 1] == "}")) {
                                busqueda_multiple = true;
                                var cadena_textos_busqueda = filtro_busqueda.slice(2, -1);
                                textos_busqueda = cadena_textos_busqueda.split(",");
                            }
                        }
                    }
					...
					setTimeout(function() {
						...
						if (searchV == "") {
							...
							toSearch.each(function() {
								var	texto_elemento_lista = $(this).text();

                                // Nota: Búsqueda múltiple
                                var indice_texto_busqueda = null;
                                var anyadir_texto = false;
                                if (busqueda_multiple == false) {
                                    if (o.caseSensitive) {
                                        indice_texto_busqueda = texto_elemento_lista.indexOf(filtro_busqueda);
                                    }
                                    else {
                                        indice_texto_busqueda = texto_elemento_lista.toUpperCase().indexOf(filtro_busqueda.toUpperCase());
                                    }
                                    if (indice_texto_busqueda != -1) {
                                        anyadir_texto = true;
                                    }
                                }
                                else {
                                    ...
                                    for (var i = 0; i < textos_busqueda.length; i++) {
                                        var texto_busqueda = textos_busqueda[i];
                                        if (texto_busqueda == "") {
                                            continue;
                                        }
                                        if (o.caseSensitive) {
                                            indice_texto_busqueda = texto_elemento_lista.indexOf(texto_busqueda);
                                        }
                                        else {
                                            indice_texto_busqueda = texto_elemento_lista.toUpperCase().indexOf(texto_busqueda.toUpperCase());
                                        }
                                        switch (tipo_busqueda_multiple) {
                                            case '+': {
                                                if (indice_texto_busqueda != -1) {
				...
				$(this).next().find('.ms2side__options, .ms2side__updown').each(function() {
                    // Nota: se baja la altura de los botones (la altura del filtro) para que quede alineado (sólo si hay filtro)
                    if (o.search != false) {
                        var top = 25.6;
				...
				allSel.change(function() {
                    ...
                    //alert("selectSx.size = '" + selectSx.length + "', selectDx.size = '" + selectDx.length + "', o.maxSelected = '" + o.maxSelected + "'");
					if (selectSx.length == 0 ||
                            (o.maxSelected >= 0 && ((selectSx.length + selectDx.length) > o.maxSelected))) {
				...
				rightSel.dblclick(function () {
                    ...
                    // Nota: Eliminado borrado de filtro automático
					//removeFilter.click();
				});
