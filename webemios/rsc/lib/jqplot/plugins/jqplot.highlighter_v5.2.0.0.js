 ...
 */
(function($) {
    // https://bitbucket.org/cleonello/jqplot/issue/481/tooltip-not-always-going-away-with
    ...
    $.jqplot.Highlighter = function(options) {
        ...
        // Nota: Modificado para poder formatear diferente los ticks de los ejes y los del tooltip
    ...
    function draw(plot, neighbor) {
        ...
        // http://stackoverflow.com/questions/14441147/jqplot-highlighter-misplaced-on-bar-charts
        // https://www.openhub.net/p/jqplot/commits/421306643?query=
    ...
    function showTooltip(plot, series, neighbor) {
        ...
        if (opts.useAxesFormatters) {
            ...
            if ('' != hl.xAxisFormatString) {
                ...
                xf = $.jqplot.DefaultTickFormatter;
            }
            if ('' != hl.yAxisFormatString) {
                yfstr = hl.yAxisFormatString;
                yf = $.jqplot.DefaultTickFormatter;
            }

            var str;
            var xstr = xf(xfstr, neighbor.data[0]);

            // Nota: Se eliminan los segundos si el formato de fecha y hora es con segundos y los segundos son 0
            if (xfstr.endsWith("%H:%M:%S") == true) {
                if (xstr.substr(xstr.length - 3) == ":00") {
                    xstr = xstr.substring(0, xstr.length - 3);
                }
            }

            var ystrs = [];
            if (opts.useXTickMarks && series._xaxis._ticks[(neighbor.data[0])-1]) {
            	xstr = xf(xfstr, series._xaxis._ticks[(neighbor.data[0])-1].label);
        	}
            for (var i=1; i<opts.yvalues+1; i++) {
				if(opts.useYTickMarks && series._yaxis._ticks[neighbor.data[1]])
					ystrs.push(yf(yfstr, series._yaxis._ticks[neighbor.data[1]].label));
				else
                    // Nota: Si es 'null' es establece el valor a 'ND'
                    var valor = yf(yfstr, neighbor.data[i]);
                    try {
                        if (valor == "null") {
                            valor = TLNT.Idiomas._("ND");
                        }
                    } catch (err) {}
					ystrs.push(valor);
            }
